...
router.post("/insert-account-movement-revolut", (req, res) => {
  ...
  var sql = "INSERT INTO revolut_account_activity (date_mov, type, name, asset_type, quantity, price, value, `return`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  con.query(sql, [date, type, name, assetType, quantity, price, value, req.body.return], function(err, result) {
    if (err) {
      console.log(err);
...
router.post("/update-account-movement-revolut", (req, res) => {
  ...
  var sql = "UPDATE revolut_account_activity SET date_mov = ?, type = ?, name = ?, asset_type = ?, quantity = ?, price = ?, value = ?, `return` = ? WHERE id = ?";
...
router.post("/insert-portfolio-snapshot-revolut", async (req, res) => {
  ...
  try {
    ...
    for (var i in positions) {
      var sql2 = "INSERT INTO revolut_portfolio_snapshot_positions (snapshot_id, name, asset_type, price, quantity, value, `return`) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
router.post("/update-portfolio-snapshot-revolut", async (req, res) => {
  if (!req.session.isLoggedIn) {
    res.json({status: "NOK", error: "Invalid Authorization."});
    return;
  }
  ...
  try {
    ...
    await connection.query(
      "UPDATE revolut_portfolio_snapshot_headers SET balance = ?, profit = ? WHERE id = ?",
      [balance, profit, snapshotId]
    );
    await connection.query(
      "DELETE FROM revolut_portfolio_snapshot_positions WHERE snapshot_id = ?",
      [snapshotId]
    );

    for (var i in positions) {
      await connection.query(
        "INSERT INTO revolut_portfolio_snapshot_positions (snapshot_id, name, asset_type, price, quantity, value, `return`) VALUES (?, ?, ?, ?, ?, ?, ?)",
